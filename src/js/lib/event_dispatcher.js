...
const EventRegistry = {
  AnnotationState: {
    ...
    setAnnotations: (event, _this) => {
      _this.setAnnotationsFromData(event.detail.annotations);
    },
    editAnnotation: (event, _this) => {
      _this.editAnnotationById(event.detail.id, event.detail.range, event.detail.shape);
    }
  },
  Controls: {
